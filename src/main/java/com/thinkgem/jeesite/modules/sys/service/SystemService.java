...
import com.thinkgem.jeesite.modules.sys.dao.DbDao;
...
public class SystemService extends BaseService  {
	...
	public List<Mail> findMailConfigAll(){
		DetachedCriteria dc = mailDao.createDetachedCriteria();
		
		dc.add(Restrictions.eq(Mail.FIELD_DEL_FLAG, Mail.DEL_FLAG_NORMAL));
		return mailDao.find(dc);
	}
	
	public Mail getMailDefault(){
		DetachedCriteria dc = mailDao.createDetachedCriteria();
		dc.add(Restrictions.eq("asdefault", "1"));
		dc.add(Restrictions.eq(Mail.FIELD_DEL_FLAG, Mail.DEL_FLAG_NORMAL));
		List<Mail> result =  mailDao.find(dc);
		if(result.size()>0){
			return result.get(0);
		}else{
			return null;
	...
	@Transactional(readOnly = false)
	public void setToUnUse(String id){
		mailDao.setUnUse(id);
	}
	
	
	//
	public List<Db> findDbPathAll(){
		DetachedCriteria dc = dbDao.createDetachedCriteria();
		
		dc.add(Restrictions.eq(Db.FIELD_DEL_FLAG, Db.DEL_FLAG_NORMAL));
		return dbDao.find(dc);
	}
	
	public Db getDb(String id){
		return  dbDao.get(id);
	}
	
	@Transactional(readOnly = false)
	public void saveMail(Db db)
	{
		dbDao.save(db);
	}
	@Transactional(readOnly = false)
	public void deleteDbPath(String id){
		dbDao.deleteById(id);
	}
	
	public Page<Db> findDb(Page<Db> page, Db db) {
		DetachedCriteria dc = dbDao.createDetachedCriteria();
		dc.add(Restrictions.eq(IssueInfo.FIELD_DEL_FLAG, IssueInfo.DEL_FLAG_NORMAL));
		return dbDao.find(page, dc);
	}
